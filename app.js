...
// Apply branding + theme from config.json.
function applyConfig(cfg) {
  if (!cfg) return;
  const t = cfg.theme || {};
  const root = document.documentElement.style;
  const map = { bg: "--off-white", card: "--white", ink: "--dark-blue", accent: "--light-blue", highlight: "--yellow-green", live: "--orange" };
  for (const [k, v] of Object.entries(map)) if (t[k]) root.setProperty(v, t[k]);
  if (t.ink) root.setProperty("--black", t.ink);
  const set = (id, txt) => { const el = document.getElementById(id); if (el && txt) el.textContent = txt; };
  set("brandName", cfg.logoText || cfg.poolName);
  set("editionTag", cfg.edition || cfg.competition?.name);
  set("heroKicker", cfg.competition?.name || "Survival Pool");
  set("poolName", cfg.poolName);
  set("tagline", cfg.tagline);
  const title = `${cfg.poolName || "Survival Pool"}${cfg.edition ? " — " + cfg.edition : ""}`;
  document.title = title;
  set("footNote", `Auto-updates from live results${cfg.competition?.finalLabel ? " · " + cfg.competition.finalLabel : ""}`);
}

// Before the first updater run, render a preview straight from the roster (everyone alive).
function previewFromRoster(roster) {
  const people = (roster.people || []).slice().sort((a, b) => (a.group || "").localeCompare(b.group || "") || a.name.localeCompare(b.name));
  return {
    updatedAt: "", stage: "Not started", aliveCount: people.length, eliminatedCount: 0, champion: null,
    recentResults: [], bracket: null, matches: null,
    standings: people.map((p, i) => ({ ...p, status: "alive", stageReached: "group", eliminatedAt: null, played: 0, points: 0, gd: 0, form: "-", statusLabel: "Yet to play", rank: i + 1 })),
  };
}

async function load() {
  try {
    try { applyConfig(await (await fetch("./config.json?t=" + Date.now())).json()); } catch { /* config optional */ }
    let data;
    try {
      const res = await fetch("./data/standings.json?t=" + Date.now());
      if (!res.ok) throw new Error("no standings yet");
      data = await res.json();
      if (!data.standings || !data.standings.length) throw new Error("empty");
    } catch {
      const roster = await (await fetch("./data/roster.json?t=" + Date.now())).json();
      data = previewFromRoster(roster);
    }
    renderMeta(data);
    renderMatches(data);
    renderBoard(data);
    ...
    document.getElementById("board").innerHTML =
      `<li class="row"><div class="who"><div class="name">Couldn't load</div><div class="team">${escapeHtml(e.message)}</div></div></li>`;
  } finally {
