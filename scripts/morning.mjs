...
const ESPN_BASE = "https://site.api.espn.com/apis/site/v2/sports/soccer";  // league slug appended from config.json
const TZ = "America/Los_Angeles";  // change to your pool's timezone
...
async function main() {
  ...
  if (!events.length) {
    await postSlack(`☀️ *Good morning!* No games today — enjoy the rest day. 😴⚽`);
    return;
  }

  const lines = [`☀️ *Today's matches* — ${dateLabel}`, ""];
  ...
  lines.push(`Good luck out there!${cfg.siteUrl ? ` Standings → ${cfg.siteUrl}` : ""}`);
