...
async function fetchTournamentState(roster) {
  ...
  const prompt = `You are a precise sports-data assistant. Today's date is provided by the system. Use the web_search tool to get the CURRENT state of the ${COMP.name || "tournament"}. Cross-check at least two reputable sources (ESPN, the official competition site, BBC Sport, Wikipedia). Never invent results.

For EACH of these ${roster.people.length} teams, determine its current tournament status:
${teamLines}

Tournament format: ${COMP.format || "group stage followed by a knockout bracket."} A team is "eliminated" once it is mathematically out (finishes the group stage without advancing, or loses a knockout match).
